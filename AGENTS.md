...
- **`packages/netior-core`** (`@netior/core`) — DB 로직 (connection, repositories, migrations). tsup (ESM+CJS). desktop-app과 narre-server가 공유.
- **`packages/netior-mcp`** (`@netior/mcp`) — MCP 서버. moc-core를 17개 도구로 래핑. stdio transport (Codex 등록 가능).
- **`packages/narre-server`** (`@netior/narre-server`) — Narre AI 에이전트. Anthropic SDK + Express. SSE 스트리밍. 세션 관리.
