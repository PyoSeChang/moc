...
- [ ] narre-server가 netior-mcp에 HTTP로 연결하는지 확인.
...
- [ ] narre-server가 SSE로 응답을 스트리밍하는지 확인.
...
- [ ] narre-server 연결 실패 시 에러 메시지가 표시되는지 확인.
...
- [ ] narre-server 응답 중 네트워크 끊김 → 에러 메시지 표시, 앱 크래시 없음.
