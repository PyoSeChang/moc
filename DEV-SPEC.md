...
```
packages/
├── shared/           타입, 상수 공유 (desktop-app ↔ agent-server)
├── desktop-app/      Electron 데스크탑 앱
└── agent-server/     MCP/Agent 서버 (MVP 이후)
```

agent-server가 향후 추가될 것이기 때문에 모노레포로 시작한다. shared 패키지를 통해 타입을 공유한다.
...
| 기능 | 비고 |
|------|------|
| MCP 서버 | agent-server 패키지. Agent가 SQLite 메타데이터 참조 |
